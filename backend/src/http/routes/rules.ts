...
import { ensureNumber, ensureObject, ensureOptionalString, ensureString } from "@/http/validation.js";
...
const parseRuleInput = (body: Record<string, unknown>) => {
  ...
  const scoreCalculation = ensureString(body.scoreCalculation, "scoreCalculation");
  if (!["decimal", "fiveDropSixUp", "round", "floor", "ceil"].includes(scoreCalculation)) {
  ...
  return {
    ...
    uma: {
      ...
      fourth: uma.fourth === null || uma.fourth === undefined ? null : ensureNumber(uma.fourth, "uma.fourth"),
    },
    oka: {
    ...
    scoreCalculation: scoreCalculation as "decimal" | "fiveDropSixUp" | "round" | "floor" | "ceil",
  };
};
...
export const buildRulesRouter = (services: Services) => {
  ...
  app.get("/:ruleId", async (c) => ok(c, await services.ruleService.getRule(c.req.param("ruleId"))));
  app.post("/", async (c) => {
    const body = ensureObject(await c.req.json(), "body");
    return ok(c, await services.ruleService.createRule(parseRuleInput(body)), 201);
  });
  app.patch("/:ruleId", async (c) => {
    const body = ensureObject(await c.req.json(), "body");
    const patch: Record<string, unknown> = {};
    if (body.name !== undefined) patch.name = ensureString(body.name, "name");
    if (body.description !== undefined) patch.description = ensureString(body.description, "description");
    if (body.gameType !== undefined) patch.gameType = ensureString(body.gameType, "gameType");
    if (body.scoreCalculation !== undefined) patch.scoreCalculation = ensureString(body.scoreCalculation, "scoreCalculation");
    ...
    return ok(c, await services.ruleService.updateRule(c.req.param("ruleId"), patch));
  });
