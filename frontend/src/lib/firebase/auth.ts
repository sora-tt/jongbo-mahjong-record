...
const ensureFirebaseAuthInitialized = async () => {
  ...
  authInitPromise = (async () => {
    if (!authEmulatorConfigured && process.env.NEXT_PUBLIC_FIREBASE_AUTH_EMULATOR_URL) {
...
export const loginWithEmail = async (email: string, password: string) =>
  signInWithEmailAndPassword(await ensureFirebaseAuthInitialized(), email, password);

export const logout = async () => signOut(await ensureFirebaseAuthInitialized());

export const getCurrentUser = async () => (await ensureFirebaseAuthInitialized()).currentUser;
