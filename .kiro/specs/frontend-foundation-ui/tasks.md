...
- [ ] 1.1 BE `AppType`由来のFE contractsと共通値型を固定する
...
- [ ] 1.2 共通Hono clientとdata/error/204 parserを実装する
...
- [ ] 1.3 DTO adapterとfeature APIの利用境界を適用する
...
- [ ] 2.1 session exchangeとCookie-only認証を統一する
...
- [ ] 2.2 loading/error/emptyとstale requestを共通化する
...
- [ ] 3.1 (P) semantic design tokenとclass composition方針を整備する
...
- [ ] 3.2 (P) accessible form・action・surface・table primitivesを実装する
...
- [ ] 3.3 (P) Loading/Error/EmptyとAuthFormShellを実装する
...
- [ ] 4.1 Header、navigation、AppShellを共通UIへ移行する
...
- [ ] 4.2 layout、providers、auth routeへのshell適用を検証する
...
- [ ] 5.2 foundation検証と後続feature handoffを実施する
