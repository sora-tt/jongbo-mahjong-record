...
- [x] 1.1 (P) 個人成績・user statsのtyped APIとadapterを整備する
...
- [x] 1.2 (P) standings・records・daily・progression APIとadapterを整備する
...
- [x] 2.2 Season/League standingsとrecordsを共通Tableへ移行する
...
- [x] 3.1 日次記録のBE契約を確認し、point progressionをtyped APIへ接続する
  - BEが返すpoint progressionのseriesを表示modelへ渡す。日次記録はBE APIが存在しないため、FEで集計・推測せず旧未使用UIを退役する。
  - 空、部分、null、未計算を区別し、0埋め・平均・補間を追加しない。
  - _Depends: 1.2_
  - _Requirements: 3.1-3.3, 4.1-4.3, 5.1_
  - _Boundary: Daily/Progression UI_

- [x] 3.2 チャートを共通primitiveとdesign tokenへ統合する
...
- [x] 4.1 統計画面のhookとAsyncStateを統合する
...
- [x] 4.2 route、navigation、metadata、Auth boundaryを整理する
...
- [x] 5.1 対象routeのmock、直接fetch、手書き型を除去する
...
- [x] 5.2 重複hooks/componentsと不要Reduxを安全に退役する
...
- [x] 5.3 未接続操作とconsole-only完了を除去する
...
- [x] 6.1 API契約とadapter/unit検証を追加する
...
- [x] 6.2 主要画面のloading/error/empty/smoke検証を行う
...
- [x] 6.3 typecheck、lint、buildとlegacy静的scanを完了する
...
- BEには日次専用の統計APIが存在しないため、FEで日次集計を新設せず、旧daily-record mock・tableを削除した。日次表示を追加する場合はBE契約を先に整備する。
- Session/Match記録フローで利用中のRedux stateは保持し、未使用だったLeague/Match/User sliceとselectorのみ削除した。
